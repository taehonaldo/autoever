...
import { getFAQ, postViewCount } from '../../../apis';
...
const FAQContent = ({ tab, category, keyword, getDataLength }: FAQContentProps) => {
	...
	return (
		<ComponentWrapper>
			...
			{canLoadMore && <LoadMore onClick={() => setOffset((prev) => prev + 10)}>+ 더보기</LoadMore>}
			{empty && (
				<Empty
					text='검색 결과가 없습니다.'
...
const FAQHeader = styled.div<{ $expanded: boolean }>`
	...
	background-color: ${(props) => (props.$expanded ? 'var(--gray-10)' : 'white')};
