...
type termKey = keyof typeof termData;
...
export const handlers = [
  ...
  http.get("/api/faq/category", ({ request }) => {
    ...
    if (!tab || !(tab in categoryData)) {
      return HttpResponse.json(
        { error: `Invalid or missing tab value: ${tab}` },
        { status: 400 }
      );
    }
  ...
  http.get("/api/faq", ({ request }) => {
    ...
    if (isNaN(offset) || offset < 0) {
      return HttpResponse.json(
        { error: `Invalid offset value: ${offsetParam}` },
        { status: 400 }
      );
    }
    if (isNaN(limit) || limit <= 0) {
      return HttpResponse.json(
        { error: `Invalid limit value: ${limitParam}` },
        { status: 400 }
      );
    }
  ...
  http.get("/api/terms", ({ request }) => {
    const url = new URL(request.url, "http://localhost");
    const type = url.searchParams.get("termsClassID");

    const returnData = termData[type as termKey];

    if (!returnData) {
      return HttpResponse.json(
        { error: `Invalid termsClassID value: ${type}` },
        { status: 400 }
      );
    }
